...
void main() {
  group('ThemeBloc State Management Tests', () {
    ...
    test('Initial state of ThemeBloc should be default Cathedral light mode', () {
    ...
    blocTest<ThemeBloc, ThemeState>(
      ...
      build: () => ThemeBloc(),
      act: (bloc) => bloc.add(const ChangeThemeEvent(
        themeMode: AppThemeMode.oliveGrove,
        isDarkMode: true,
      )),
      expect: () => [
        predicate<ThemeState>((state) {
          return state.themeMode == AppThemeMode.oliveGrove && state.isDarkMode == true;
        }),
      ],
    );
  });
